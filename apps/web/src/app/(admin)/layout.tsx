...
import { LayoutDashboard, Building2, Users, BookOpen, CheckSquare, BarChart3, DollarSign, Shield, Settings, LogOut, Bell, ChevronDown, Cpu, Menu, X } from 'lucide-react'
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className={`fixed left-0 top-0 h-full w-64 flex flex-col z-40 border-r border-border/50 transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`} style={{ background: 'rgba(7, 8, 18, 0.98)', backdropFilter: 'blur(20px)' }}>
        ...
        <div className="border-t border-border/50 p-3 space-y-1">
          <Link href="/admin/settings" className="nav-item" onClick={() => setSidebarOpen(false)}><Settings className="h-4 w-4" /><span>System Settings</span></Link>
      ...
      <div className="flex-1 lg:pl-64">
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-border/50 px-4 sm:px-6" style={{ background: 'rgba(7, 8, 18, 0.9)', backdropFilter: 'blur(20px)' }}>
          <button
          ...
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            <span className="text-muted-foreground hidden sm:inline">System: <span className="text-emerald-400">Healthy</span></span>
          </div>
